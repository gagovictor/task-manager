...
const ThreeScene: React.FC<ThreeSceneProps> = ({
    ...
    const disposeScene = () => {
        // Dispose of nodes, lines, and geometries
        nodes.current.forEach((node) => node.geometry.dispose());
        lines.current.forEach((line) => line.geometry.dispose());
        lineGeometries.current.forEach((geometry) => geometry.dispose());
        particles.current?.geometry.dispose();

        // Clear the scene and refs
        nodes.current = [];
        lines.current = [];
